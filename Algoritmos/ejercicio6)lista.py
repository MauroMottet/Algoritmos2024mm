super_heroes = [
  ...
  },
  {
    ...
    "año_aparicion": 1962,
    "casa_comic": "Marvel Comics",
    "biografia": "Hank Pym o Scott Lang, héroes capaces de cambiar de tamaño y comunicarse con insectos, Usa un Traje."
  }
]
...
criterio = "nombre"
eliminado = remove(super_heroes,criterio,buscado)
print("se elimino de la lista el super heroe: ",eliminado )
...
if index is not None:
    ...
    print("no existe")
index2 = search (super_heroes,"nombre","Doctor Strange")
super_heroes[index2]["casa_comic"] = "Marvel Comics"
print(super_heroes[index2])
for  personaje in super_heroes:
    if "armadura"  in personaje["biografia"] or "Traje"  in personaje["biografia"]: 
       print(personaje)
for heroes in super_heroes:
    if heroes["año_aparicion"] < 1963 :
        print(heroes)
index3 = search(super_heroes,"nombre","Capitana Marvel")
index4 = search(super_heroes,"nombre","Mujer Maravilla")
print(f"la mujer maravilla pertenece a la casa {super_heroes[index4]["casa_comic"]}")
print(f"la capitana marvel pertenece a la casa {super_heroes[index3]["casa_comic"]}")
index5 = search(super_heroes,"nombre","Flash")
index6 = search(super_heroes,"nombre","Star-Lord")
print(super_heroes[index5])
print(super_heroes[index6])

for heroe in super_heroes:
    if heroe["nombre"].startswith("B") or heroe["nombre"].startswith("M") or heroe["nombre"].startswith("S"):
        print(heroe)
c=0
c1=0

for heroes in super_heroes:
    if heroes["casa_comic"] == "Marvel Comics" :
        c+=1
    if heroes["casa_comic"] == "DC Comics":
        c1+=1
print("La casa Marvel Comics tiene: ", c,"Super heroes y la casa DC Comics tiene: ",c1 ,"Super Heroes")